...
// 创建行星样式 - 更具特色
export function createPlanet(body) {
    ...
    if (Math.random() > 0.3) {
        ...
        const ringGeometry = new THREE.RingGeometry(ringSize - ringWidth, ringSize, 64);
        // 随机环颜色 - 通常与行星基色相关但更亮
        const baseColor = new THREE.Color(body.color);
        const ringColor = new THREE.Color();
        const hsl = { h: 0, s: 0, l: 0 };
        baseColor.getHSL(hsl);
        ringColor.setHSL((hsl.h + THREE.MathUtils.randFloatSpread(0.2) + 1) % 1, THREE.MathUtils.randFloat(0.5, 0.8), THREE.MathUtils.randFloat(0.6, 0.8));
    ...
    if (Math.random() > 0.7) {
        ...
        for (let i = 0; i < moonCount; i++) {
            ...
            const hsl = { h: 0, s: 0, l: 0 };
            baseColor.getHSL(hsl);
            const moonColor = new THREE.Color().setHSL(hsl.h, THREE.MathUtils.randFloat(0.1, 0.3), THREE.MathUtils.randFloat(0.7, 0.9));
            ...
            moon.position.set(Math.cos(moonAngle) * Math.cos(moonElevation) * moonDistance, Math.sin(moonElevation) * moonDistance, Math.sin(moonAngle) * Math.cos(moonElevation) * moonDistance);
    ...
    planetGroup.userData.rotationAxis = new THREE.Vector3(THREE.MathUtils.randFloatSpread(0.2), 1, THREE.MathUtils.randFloatSpread(0.2)).normalize();
    return planetGroup;
}
// 创建恒星样式 - 更具特色
export function createStar(body) {
    ...
    const glowColor = new THREE.Color().setHSL(hsl.h, Math.min(1, hsl.s * 1.2), Math.min(1, hsl.l * 1.5));
...
export function createBlackHole(body) {
    ...
    for (let i = 0; i < diskVertexCount; i++) {
        ...
        // 内部更亮（偏白热），外部带有颜色
        const vertexColor = new THREE.Color().setHSL(adjustedH + t * 0.1, 0.9 - t * 0.7, 0.9 - t * 0.3);
    ...
    for (let i = 0; i < particlesCount; i++) {
        ...
        const particleColor = new THREE.Color().setHSL(adjustedH + t * 0.1, 0.9 - t * 0.7, 1 - t * 0.4);
